...
const Index = () => {
  ...
  const handleAddPerson = () => {
    if (firstName.trim() && lastName.trim()) {
      ...
      const activeIncidentId = incidents.find(inc => inc.person === '')?.id || incidents[incidents.length - 1].id;
  ...
  return <div className="container mx-auto py-20 px-4 max-w-6xl">
      ...
      <GuardianContactModal isOpen={isGuardianDialogOpen} onOpenChange={setIsGuardianDialogOpen} people={incidents.map(i => [i.person, i.perpetrator]).flat().filter(p => p && p.length > 0).filter((v, i, a) => a.indexOf(v) === i)} onSubmit={submitForm} />

      {/* Add Person Modal */}
      <AddPersonModal isOpen={isAddPersonDialogOpen} onOpenChange={setIsAddPersonDialogOpen} onAddPerson={handleAddPerson} firstName={firstName} setFirstName={setFirstName} lastName={lastName} setLastName={setLastName} personalNumber={personalNumber} setPersonalNumber={setPersonalNumber} gender={gender} setGender={setGender} classGroup={classGroup} setClassGroup={setClassGroup} genderOptions={genderOptions} classOptions={classOptions} />
    </div>;
};
